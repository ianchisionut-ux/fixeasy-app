  ...
  return (
    <>
      ...
      <div className="hero" style={{ padding: "56px 24px" }}>
        <span className="eyebrow"><cat.icon size={14} strokeWidth={2.2} style={{ verticalAlign: -2 }} /> {cat.label}</span>
        ...
        <div className="prov-grid">
          {citiesResult.rows.map((row) => (
            <a key={row.city} href={`/${cat.slug}/${citySlug(row.city)}`} className="prov-card" style={{ padding: 20, textDecoration: "none" }}>
              <div style={{ color: "var(--steel)", marginBottom: 10 }}><cat.icon size={28} strokeWidth={1.8} /></div>
